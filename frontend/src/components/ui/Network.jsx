...
function NetworkVisualization() {
    ...
    const dimensions = useMemo(() => {
        ...
        const scale = baseSize / 900;   // makes everything larger
        return {
    ...
    useEffect(() => {
        const updateSize = () => {
            ...
            const size = Math.min(width - 32, height - 32); // no cap
            setContainerSize({
                width: size,
                height: size
            });
    ...
    const avatars = useMemo(() => {
        const newImageLinks = ["https://i.postimg.cc/W1rCvYnT/nazmul-hossain.jpg", "https://i.pinimg.com/736x/8c/6d/db/8c6ddb5fe6600fcc4b183cb2ee228eb7.jpg", "https://i.pinimg.com/736x/6f/a3/6a/6fa36aa2c367da06b2a4c8ae1cf9ee02.jpg", "https://i.pinimg.com/1200x/c2/4e/27/c24e271f2f992fd7e62e8c1e8d9b3e2f.jpg", "https://i.pinimg.com/736x/81/d6/b1/81d6b158728f5fc97ca6e0a025fefee0.jpg", "https://i.pinimg.com/736x/9f/46/74/9f4674ca9c17330ab419c1b2f5951d9a.jpg", "https://i.pinimg.com/736x/57/3c/80/573c80967c9429d0ed0ce32701f85b70.jpg", "https://i.pinimg.com/736x/b0/c4/21/b0c421e77cf563962026ade82c90dd5b.jpg", "https://i.pinimg.com/736x/ce/31/42/ce3142d7a968fff3aecd0100572a5e8b.jpg", "https://i.pinimg.com/736x/79/63/a5/7963a5246188d408b8f28961a0cf2b90.jpg", "https://i.pinimg.com/736x/8e/c1/f8/8ec1f80db272047cedf4c20263114387.jpg", "https://i.pinimg.com/1200x/08/a2/41/08a2413b771b729a9f9df20fa97be52a.jpg", "https://i.pinimg.com/736x/b0/7b/cc/b07bcc19e5d06dfb888c3263724b8baa.jpg", "https://i.pinimg.com/736x/12/ec/d9/12ecd918607b1ccb9d46772435bb592f.jpg", "https://i.pinimg.com/1200x/e2/f5/bc/e2f5bc45bd9d07946c9453cfb48747ea.jpg", "https://i.pinimg.com/1200x/50/47/d2/5047d259f0d8b3d652b7d3dfa3479139.jpg", "https://i.pinimg.com/736x/bb/87/18/bb87180897cb4cb694cd692966a0ab15.jpg", "https://i.pinimg.com/1200x/fb/c3/03/fbc30308d8f36a5566cbf0a535c14322.jpg", "https://i.pinimg.com/1200x/de/11/d2/de11d2f9df4295493625189e9cb829ce.jpg"];
        return [{
            id: 1,
            name: "Zane Whitaker",
    ...
    return (<div className="flex items-center justify-center p-2 sm:p-4 overflow-hidden font-sans w-full h-[70vh]">
        { }
        <div className="relative" style={{
            ...
            { }
            <div className="absolute border-2 border-dashed border-purple-400/50 rounded-full top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2" style={{
            ...
            { }
            <div className="absolute shadow-lg z-10 group-center cursor-pointer" style={{
                left: `${dimensions.centerX}px`,
                top: `${dimensions.centerY}px`,
                transform: `translate(-50%, -50%)`
            }}>
                <img src="https://i.pinimg.com/736x/5c/62/7a/5c627a3458297ee0c587328e5f7061fc.jpg" alt="Center Image" className="rounded-full object-cover border-4 border-white" style={{
              ...
              ${isCurrentlyConnected('center') ? 'opacity-100' : 'opacity-0 group-center:hover:opacity-100'}
          `}>
                    Center Hub
                    { }
            ...
            <svg className="absolute inset-0 w-full h-full z-0">
                ...
                <AnimatePresence>
                    {activeConnections.map(conn => {
                        ...
                        return <motion.line key={`${conn.from}-${conn.to}`} x1={fromPos.cx} y1={fromPos.cy} x2={toPos.cx} y2={toPos.cy} stroke={conn.color.includes('blue') ? '#3b82f6' : conn.color.includes('green') ? '#10b981' : conn.color.includes('purple') ? '#8b5cf6' : conn.color.includes('yellow') ? '#f59e0b' : conn.color.includes('red') ? '#ef4444' : conn.color.includes('orange') ? '#f97316' : conn.color.includes('pink') ? '#ec4899' : conn.color.includes('cyan') ? '#06b6d4' : conn.color.includes('white') ? '#ffffff' : '#3b82f6'} strokeWidth={dimensions.strokeWidth} strokeOpacity="0.8" fill="none" initial={{
            ...
            {outerRingAvatars.map(avatar => {
                const {
                    cx,
                    cy
                } = allAvatarPositions[avatar.id];
                const isActive = isCurrentlyConnected(avatar.id);
                return <div key={avatar.id} className="absolute" style={{
                    left: `${cx}px`,
                    top: `${cy}px`,
                    transform: `translate(-50%, -50%)`
                }}>
                    { }
                    <div className={`bg-white rounded-full flex items-center justify-center shadow-lg hover:scale-105 transition-transform duration-200 cursor-pointer group-outer-${avatar.id} relative`} style={{
                        width: `${dimensions.outerAvatarSize}px`,
                        height: `${dimensions.outerAvatarSize}px`
                    }}>
                        { }
                        <div className="rounded-full relative overflow-hidden flex items-center justify-center" style={{
                            width: `${dimensions.outerImageSize}px`,
                            height: `${dimensions.outerImageSize}px`
                        }}>
                            { }
                            <img src={avatar.image || "/placeholder.svg"} alt={avatar.name} className="w-full h-full object-cover rounded-full" onError={e => {
                                const target = e.target;
                                target.style.display = "none";
                                const parent = target.parentElement;
                                if (parent) {
                        ...
                        { }
                        <div className={`absolute -top-10 left-1/2 transform -translate-x-1/2 bg-white/90 backdrop-blur-sm px-3 py-1 rounded-full font-medium text-gray-800 shadow-lg transition-opacity duration-200 pointer-events-none whitespace-nowrap z-20 ${dimensions.tooltipTextSize}
                    ${isActive ? 'opacity-100' : 'opacity-0 group-outer-${avatar.id}:hover:opacity-100'}
                `}>
                            {avatar.name}
                            { }
                            <div className="absolute top-full left-1/2 transform -translate-x-1/2 w-0 h-0 border-l-4 border-r-4 border-t-4 border-transparent border-t-white/90"></div>
            ...
            {innerRingAvatars.map(avatar => {
                ...
                return <div key={avatar.id} className="absolute" style={{
                    ...
                    { }
                    <div className={`bg-white rounded-full flex items-center justify-center shadow-lg hover:scale-105 transition-transform duration-200 cursor-pointer group-inner-${avatar.id} relative`} style={{
                        ...
                        { }
                        <div className={`absolute -top-10 left-1/2 transform -translate-x-1/2 bg-white/90 backdrop-blur-sm px-3 py-1 rounded-full font-medium text-gray-800 shadow-lg transition-opacity duration-200 pointer-events-none whitespace-nowrap z-20 ${dimensions.tooltipTextSize}
                    ${isActive ? 'opacity-100' : 'opacity-0 group-inner-${avatar.id}:hover:opacity-100'}
                `}>
                            {avatar.name}
                            { }
                            <div className="absolute top-full left-1/2 transform -translate-x-1/2 w-0 h-0 border-l-4 border-r-4 border-t-4 border-transparent border-t-white/90"></div>
